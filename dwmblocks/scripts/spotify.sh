...
if [ "$PLAYERCTL_STATUS" = "Playing" ]; then
    ...
    if [ ${#TITLE} -gt $MAX_LENGTH ]; then
        ...
        echo -e "$TRUNCATED_TITLE$RESET"
    else
        # Display the full title if within the limit
        echo -e "$TITLE$RESET"
    fi
elif [ "$PLAYERCTL_STATUS" = "Paused" ]; then
    echo -e "$NOT_PLAYING_TEXT$RESET"  # Display "Not Playing" when paused
else
    echo -e "$NOT_PLAYING_TEXT$RESET"  # Display "Not Playing" when Spotify is inactive or there's an error
fi