...
enum { SchemeNorm, SchemeSel, SchemeLay }; /* color schemes */
...
drawstatusbar(Monitor *m, int bh, char* stext) {
	...
	ret = m->ww - w - rightpadding; // Adjust ret to include right padding
	x = m->ww - w - getsystraywidth() - rightpadding; // Adjust x for right padding

	drw_setscheme(drw, scheme[LENGTH(colors)]);
	drw->scheme[ColFg] = scheme[SchemeNorm][ColFg];
	drw->scheme[ColBg] = scheme[SchemeNorm][ColBg];
	drw_rect(drw, 0, 0, m->ww, bh, 1, 1); // Fill the background area, including the padding
	// drw_rect(drw, x - rightpadding, 0, w + rightpadding, bh, 1, 1);
	x++;
...
// (status bar, tags, layout symbol) on the monitor m
void 
drawbar(Monitor *m) {
	int x, w, tw = 0, stw = 0;              // Initialize variables for x position, width, status width, and systray width
	int boxs = drw->fonts->h / 9;           // Size of the box indicator for tags, calculated based on font height
	int boxw = drw->fonts->h / 6 + 2;       // Width of the box indicator for tags
	unsigned int i, occ = 0, urg = 0;       // i for looping tags, occ for occupied tags, urg for urgent tags
	Client *c;                              // Client pointer for iterating over clients on the monitor

	// Check if the bar should be shown; if not, return and skip drawing
	if (!m->showbar)
		return;

	// If the system tray is shown, is on the current monitor, and is aligned to the right,
	// calculate the width of the system tray (stw) and reserve space for it on the bar
	if(showsystray && !systrayonleft)
		stw = getsystraywidth();

	// Set tw to the width of the monitor's screen minus the status bar width
	tw = statusw = m->ww - drawstatusbar(m, bh, stext);

	// Adjust the bar window size to fit the monitor's width
	resizebarwin(m);

	// Loop through all clients on the monitor to find which tags are occupied and which are urgent
	for (c = m->clients; c; c = c->next) {
		occ |= c->tags;                    // Update occ with the tags of each client to track which tags are occupied
		if (c->isurgent)
			urg |= c->tags;               // If a client is urgent, update urg with its tags to track urgent tags
	}

	x = 0; // Initialize x position for drawing tags on the bar

	// Loop through each tag and draw it on the bar
	for (i = 0; i < LENGTH(tags); i++) {
		w = TEXTW(tags[i]);                // Calculate width for each tag based on the text width of tags[i]
		// Set color scheme based on whether the tag is selected or not
		drw_setscheme(drw, scheme[m->tagset[m->seltags] & 1 << i ? SchemeSel : SchemeNorm]);
		// Draw the tag text on the bar
		drw_text(drw, x, 0, w, bh, lrpad / 2, tags[i], urg & 1 << i);
		x += w;                            // Move x to the right for the next tag
	}

	// Draw the layout symbol on the far right of the bar
	w = TEXTW(m->ltsymbol);                 // Get the text width of the layout symbol
	drw_setscheme(drw, scheme[SchemeLay]); // Use normal color scheme for the layout symbol
	x = drw_text(drw, x, 0, w, bh, lrpad / 2, m->ltsymbol, 0); // Draw the layout symbol text and update `x`
	
	// If space is left, draw the selected client's name in the middle of the bar
	if ((w = m->ww - tw - stw - x) > bh) { // Check if there's enough width left to draw client name
		if (m->sel) {                      // If a client is selected
			drw_setscheme(drw, scheme[SchemeNorm]); // Always use normal scheme for both background and text
			drw_text(drw, x, 0, w, bh, lrpad / 2, m->sel->name, 0); // Draw selected client's name in the remaining space
		} else {                           // If no client is selected, clear this section of the bar
			drw_setscheme(drw, scheme[SchemeNorm]);
			drw_text(drw, x, 0, w, bh, lrpad / 2, "", 0); // Clear text by drawing an empty string
		}
	}

	// Map the entire bar to the screen, drawing it on the barwin window
...
updatestatus(void) 
{
	...
	// Loop through each monitor and update its bar
